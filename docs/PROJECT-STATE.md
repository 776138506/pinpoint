...
### 2026-08-25：点击即开评论窗——截图异步回填（用户要求 ms 级）

- 根因：此前 addMark/openMark 排在 html2canvas await 之后，点击→弹窗的延迟=整张截图耗时（长页面秒级）
- 改为：捕获后同步建 mark + 开评论窗（弹窗首帧显示「截图生成中…」），截图完成后 updateMark 异步回填；暂存/发送/了结草稿会等在途截图，保证暂存区与 dsh 必带图；等待期间标记被删则不再暂存
- 截图回填重渲染不清空正在输入的评论（同一标记且不 busy 时恢复 textarea 值）
- 连带修真 bug：框选完成即暂停标记，紧随的 click 到达时标记已关——suppressClick 拦截不能再要求标记态，否则框选起于链接照样跳转
- 验证：151/151（新增 3 用例：弹窗不等截图 / 暂存等在途截图 / 回填不清评论）；npm run build 绿；实机速度未验

### 2026-08-25：评论窗关闭按钮默认暂存（用户拍板）

- × 关闭草稿从「删除」改为「了结」：有评论进暂存区，无评论才撤销——与 Esc 完全同语义，两个入口同一件事不再两种结果；删除按钮仍是唯一显式删除入口
- 验证：148/148（新增 2 用例：×有评论暂存 / ×无评论撤销）；npm run build 绿

### 2026-08-25：暂停中 Esc 不丢已输入评论（用户拍板）

- 评论子流程中按 Esc 从「删草稿」改为「了结草稿」：已输入评论自动进暂存区（复用 settleDraft 不变量），无评论才撤销；仍不恢复标记（明确退出优先）
- 验证：146/146（新增 2 个用例：有评论 Esc 进暂存 / 无评论 Esc 撤销）；npm run build 绿

### 2026-08-25：捕获即暂停、了结即恢复（用户拍板 + 补充：重开也暂停）

- 契约：评论窗打开（新捕获/点已标记元素重开/点角标）→ 标记输入自动暂停并同步 background；暂存/发送/删除/关闭弹窗 → 恢复标记
- 显式退出优先：Esc（暂停中按 Esc = 关弹窗+删草稿，同关闭按钮）、快捷键、侧栏按钮 → 清除暂停标记，不恢复；暂停中显式开标记 = 了结弹窗并回到标记态
- 白板优先：暂停期间进白板则放弃恢复（互斥语义大于恢复语义）；白板产出 mark 开评论窗不涉及暂停（标记本就关着），了结后回中立态
- 设计动机：消除「评论窗打开期间页面仍处于标记态」的冲突窗口（历史评论区高亮 bug 的同族根因）
- 验证：145/145（新增 8 个回归用例；测试桩 sendMessage 支持回调形式）；npm run build 绿；实机手感未验

### 2026-08-25：暂存区截图缩略图（用户要求）

- 暂存列表每项顶部渲染所指截图缩略图（限高 120px、裁顶），点击缩略图同样跳转标记位置；截图缺失（跨域失败/旧暂存）不渲染占位
- 验证：137/137（新增缩略图回归用例），npm run build 绿；实机观感未验

### 2026-08-25：实机三连修——点击跳转拦截 / 两模式互斥 / 长截图限高

- Bug1 标记态点链接直接跳转：onClick 捕获阶段此前不 preventDefault/stopPropagation，链接默认导航照走；拖拽起于链接时 mouseup 后的 click 也会导航。现标记态点击页面元素一律拦截（自身 UI 除外）
- 连带揪出真 bug：suppressClick 会存到下一次点击，若先消耗再判断自身 UI，框选后点弹窗「暂存/发送」会被吞——自身 UI 判断已前置（jsdom 多实例串扰帮提前暴露）
- Bug2 画笔与标记冲突（用户报告「点画笔后还处在标记模式，只能使用标记」）：两输入模式从「并存」改为「互斥」——进白板自动退标记并同步 background（侧栏按钮恢复），进标记自动退白板；所有标记开启路径汇聚 setMarking 一处兜底。已有标记的边框/角标在白板中保持显示（两模式互证不变），互斥的只是输入捕获态
- Bug3 长截图把暂存/发送按钮挤出视口：评论窗改 flex 列布局 + max-height calc(100vh-16px)，截图装进独立滚动容器（max-height 32vh）内部滚动，按钮恒可见
- 验证：136/136（npm test = tsc + vitest，含 6 个新回归用例），npm run build 绿；实机操作未验
...
- bug①点工具按钮/Esc 退出工具都调 renderPopup 重建评论窗，正在输入未发送的评论被冲掉 → 改原地切换激活态（deactivateDrawingTool）
- bug②画笔 mousemove 的 redrawCanvas 不带 preview，笔迹松开鼠标才显示（感觉没画上）→ 带 activeStroke 实时预览
- bug③画布 CSS height:100% 相对含工具条的 wrapper 计算，比图片高出一截 → 画布改挂只含 img 的 .dsh-point-ext-drawing-view，画点与落点不再纵向错位
- 测试 +4（评论保留×2 / 实时预览 / 画布挂点），137/137 绿


### 2026-08-25：性能优化（白板渲染合帧 + 截图快速路径）

- 背景：用户实机反馈"有点卡"。诚实说明：此前无基准测试，唯一兜底是截图超时降级纯文本
- 热点①白板：mousemove 每事件同步全量重绘 → rAF 合帧（每帧最多一次）+ 落笔采样（<2px 移动不产生新点）。ponytail 上限：笔迹极多仍可能掉帧，升级路径=双画布分层
- 热点②截图（大头）：此前一律撑整文档窗口渲染 → 视口内区域走快速路径（视口窗口 + 视口坐标裁剪，更保真且免去整文档渲染）；跨视口才用整文档慢速兜底。注意：拖拽框选的 rect 恒在视口内，慢路径只被「拖拽中滚动」和白板跨滚动场景触发
- 观测点：captureRegionInner 加 console.debug 计时（标明 viewport/full-document 路径），实机卡顿开控制台即可定位
- jsdom 无法测真实渲染耗时（无 canvas 管线），性能验证靠实机观测点；功能回归 133/133 绿
